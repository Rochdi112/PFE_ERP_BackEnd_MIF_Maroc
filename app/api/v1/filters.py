...
@router.get(
    ...
    description=(
        "Recherche avancée sur les interventions par statut, urgence, "
        "type ou technicien."
    ),
)
def filter_interventions(
    statut: Optional[StatutIntervention] = Query(
        None, description="Statut de l’intervention"
    ),
    urgence: Optional[bool] = Query(
        None, description="Filtrer par urgence (True/False)"
    ),
    type: Optional[InterventionType] = Query(
        None,
        description="Type d'intervention",
    ),
    technicien_id: Optional[int] = Query(
        None,
        description="ID du technicien affecté",
    ),
