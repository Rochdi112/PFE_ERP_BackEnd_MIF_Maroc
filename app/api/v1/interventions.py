...
@router.post(
    ...
    description=(
        "Crée une intervention préventive ou corrective. "
...
def create_new_intervention(
    ...
    # 👇 Assure un user_id valide: si absent dans le token,
    # crée/récupère via email
...
@router.get(
    ...
    description=(
        "Retourne toutes les interventions du système " "(authentification requise)"
    ),
)
...
@router.get(
    ...
    description=(
        "Récupère les détails d'une intervention par ID " "(authentification requise)"
    ),
)
...
@router.patch(
    ...
    description=(
        "Met à jour le statut (cycle de vie) de l'intervention. "
        "Action historisée avec l'utilisateur en cours."
    ),
