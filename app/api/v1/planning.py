...
@router.post(
    ...
    description=(
        "Crée un planning d'entretien pour un équipement donné. "
...
@router.get(
    ...
    description=(
        "Liste tous les plannings existants "
        "(lecture ouverte aux utilisateurs connectés)."
    ),
)
def list_all_plannings(
...
@router.patch(
    ...
    description=(
        "Permet de replanifier la prochaine date de maintenance. "
...
# Ajoute un endpoint PUT simple pour mettre à jour la fréquence
# (conforme aux tests)
...
def update_planning(
    planning_id: int, payload: dict = Body(...), db: Session = Depends(get_db)
):
