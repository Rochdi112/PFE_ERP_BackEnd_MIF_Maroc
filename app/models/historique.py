"""
Modèle HistoriqueIntervention : traçabilité des changements de statut
d'une intervention.
...
class HistoriqueIntervention(Base):
    ...
    __table_args__ = (
        Index(
            "idx_historique_intervention", "intervention_id", "user_id", "horodatage"
        ),
    )

    id: int = Column(Integer, primary_key=True, index=True)
    statut: StatutIntervention = Column(
        Enum(StatutIntervention), nullable=False, index=True, doc="Statut enregistré"
    )
    remarque: Optional[str] = Column(String, nullable=True, doc="Remarque libre")
    horodatage: datetime = Column(
        DateTime,
        default=datetime.utcnow,
        nullable=False,
        index=True,
        doc="Horodatage du changement",
    )
    ...
    intervention: "Intervention" = relationship(
        "Intervention", back_populates="historiques", lazy="select"
    )
    user: "User" = relationship("User", back_populates="historiques", lazy="select")

    def __repr__(self) -> str:
        return (
            f"<HistoriqueIntervention(id={self.id}, "
    ...
    def resume(self) -> str:
        ...
        return (
            f"{self.horodatage.strftime('%d/%m/%Y %H:%M')} - "
            f"{self.statut_label} par user {self.user_id}"
        )
