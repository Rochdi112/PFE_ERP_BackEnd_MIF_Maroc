...
class Intervention(Base):
    ...
    type_intervention = Column(
        "type", Enum(InterventionType), nullable=False, index=True
    )
    ...
    technicien_id = Column(
        Integer,
        ForeignKey("techniciens.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    client_id = Column(
    ...
    equipement: "Equipement" = relationship(
        "Equipement", back_populates="interventions", lazy="select"
    )

    technicien: Optional["Technicien"] = relationship(
        "Technicien", back_populates="interventions", lazy="select"
    )

    client: Optional["Client"] = relationship(
        "Client", back_populates="interventions", lazy="select"
    )

    contrat: Optional["Contrat"] = relationship(
        "Contrat", back_populates="interventions", lazy="select"
    )
    ...
    def cout_pieces_calcule(self) -> float:
        ...
        for piece_utilisee in self.pieces_utilisees:
            if (
                piece_utilisee.piece_detachee
                and piece_utilisee.piece_detachee.prix_unitaire
            ):
                total += (
                    float(piece_utilisee.piece_detachee.prix_unitaire)
                    / 100
    ...
    def equipement_nom(self) -> str:
        ...
        return (
            self.equipement.nom
            if self.equipement
            else f"Équipement #{self.equipement_id}"
        )
    ...
    def peut_etre_demarree(self) -> bool:
        ...
        return (
            ...
            and self.technicien_id is not None
        )
    ...
    def affecter_technicien(
        self, technicien_id: int, user_id: Optional[int] = None
    ) -> None:
        """
    ...
    def mettre_en_attente(
        self, raison: str = None, user_id: Optional[int] = None
    ) -> None:
        """
    ...
    def ajouter_piece(self, piece_detachee_id: int, quantite: int) -> None:
        ...
        existing = self.pieces_utilisees.filter_by(
        ...
        if existing:
            ...
            self.pieces_utilisees.append(nouvelle_piece)
    ...
    def calculer_sla_respect(self) -> Optional[bool]:
        ...
        duree_reelle = (
            self.date_cloture - self.date_creation
    ...
    def to_dict(
        ...
        data = {
            ...
            "derniere_modification": (
                self.derniere_modification.isoformat()
            ...
            data.update(
                {
                    ...
                    "date_affectation": (
                        self.date_affectation.isoformat()
                        if self.date_affectation
                        else None
                    ),
                    "date_debut_travaux": (
                        self.date_debut_travaux.isoformat()
                        if self.date_debut_travaux
                        else None
                    ),
                    "date_fin_travaux": (
                        self.date_fin_travaux.isoformat()
                        if self.date_fin_travaux
                        else None
                    ),
                    "date_cloture": (
                        self.date_cloture.isoformat() if self.date_cloture else None
                    ),
                    "date_archivage": (
                        self.date_archivage.isoformat() if self.date_archivage else None
                    ),
                    # Durées et performances
                    "duree_estimee": self.duree_estimee,
                    "duree_reelle": self.duree_reelle_calculee,
                    "duree_totale_incluant_attentes": (
                        self.duree_totale_incluant_attentes
                    ),
                    ...
                    # Coûts détaillés
                    "cout_estime": (
                        float(self.cout_estime) / 100 if self.cout_estime else None
                    ),
                    "cout_reel": (
                        float(self.cout_reel) / 100 if self.cout_reel else None
                    ),
                    "cout_pieces": (
                        float(self.cout_pieces) / 100 if self.cout_pieces else None
                    ),
                    "cout_main_oeuvre": (
                        float(self.cout_main_oeuvre) / 100
                        if self.cout_main_oeuvre
                        else None
                    ),
                    ...
                    "temps_ecoule_depuis_creation": int(
                        self.temps_ecoule_depuis_creation.total_seconds() / 3600
                    ),  # heures
                    "temps_ecoule_depuis_affectation": (
                        int(self.temps_ecoule_depuis_affectation.total_seconds() / 3600)
                        if self.temps_ecoule_depuis_affectation
                        else None
                    ),
                }
            )
            ...
            data.update(
                {
                    ...
                    "historiques_recents": [
                        h.to_dict() for h in list(self.historiques.limit(5))
                    ],
                    "documents_recents": [
                        d.to_dict() for d in list(self.documents.limit(5))
                    ],
