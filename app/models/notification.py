...
class Notification(Base):
    ...
    type_notification: TypeNotification = Column(
        "type",
    ...
    canal: CanalNotification = Column(
        Enum(CanalNotification), nullable=False, index=True, doc="Canal d'envoi"
    )
    contenu: Optional[str] = Column(String(1000), nullable=True, doc="Sujet/message")
    date_envoi: datetime = Column(
        DateTime,
    ...
    intervention: "Intervention" = relationship(
        "Intervention", back_populates="notifications", lazy="select"
    )
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"type='{self.type_notification.value}', "
    ...
    def resume(self) -> str:
        ...
        return (
            f"[{self.type_notification.value.upper()}] "
            f"{self.contenu[:40] if self.contenu else ''}..."
        )
    ...
    # NOTE: Préparé pour extension future (statut de lecture, audit,
    # suppression logique, etc.)
