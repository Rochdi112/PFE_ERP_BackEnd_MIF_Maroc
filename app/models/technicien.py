...
technicien_competence = Table(
    ...
    Column(
        "technicien_id",
        Integer,
        ForeignKey("techniciens.id", ondelete="CASCADE"),
        primary_key=True,
    ),
    Column(
        "competence_id",
...
class Technicien(Base):
    ...
    user: "User" = relationship("User", back_populates="technicien", lazy="select")
    ...
    def __repr__(self) -> str:
        """Représentation concise pour debugging."""
        return (
            f"<Technicien(id={self.id}, "
            f"user='{self.user.username if self.user else 'N/A'}', "
            f"équipe='{self.equipe}', "
            f"dispo='{self.disponibilite.value}')>"
        )
    ...
    def est_disponible(self) -> bool:
        ...
        return (
            self.is_active and self.disponibilite == DisponibiliteTechnicien.disponible
        )
    ...
    def taux_reussite(self) -> Optional[float]:
        ...
        reussites = 0
        for intervention in self.interventions.filter_by(
            ...
            if intervention.date_cloture:
                interventions_suivantes = self.interventions.filter(
                    ...
                    <= intervention.date_cloture + timedelta(days=7),
                ).count()
    ...
    def niveau_global(self) -> str:
        ...
        if self.est_expert or self.nb_competences >= 5:
            ...
            self.niveau_technicien == NiveauCompetence.avance
            or self.nb_competences >= 3
        ):
            return "Avancé"
    ...
    def changer_disponibilite(
        self, nouvelle_dispo: DisponibiliteTechnicien, raison: str = None
    ) -> None:
        """
    ...
    def ajouter_competence(
        ...
        niveau: NiveauCompetence = NiveauCompetence.intermediaire,
    ) -> None:
        """
    ...
    def calculer_charge_semaine(
        self, date_debut: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "temps_total_estime": sum(
                i.duree_estimee or 0 for i in interventions_semaine
            ),
            "temps_total_reel": sum(
                i.duree_reelle_calculee or 0
                for i in interventions_semaine
                if i.est_terminee
            ),
    ...
    def generer_rapport_performance(self, nb_mois: int = 6) -> Dict[str, Any]:
        ...
        return {
            ...
            "taux_completion": (
                round(
                    len([i for i in interventions_periode if i.est_terminee])
                    / len(interventions_periode)
                    * 100,
                    1,
                )
                if interventions_periode
                else 0
            ),
            ...
            "nb_urgentes_traitees": len(
                [i for i in interventions_periode if i.est_urgente]
            ),
            ...
            data.update(
                {
                    ...
                    "date_embauche": (
                        self.date_embauche.isoformat() if self.date_embauche else None
                    ),
                    "date_fin_contrat": (
                        self.date_fin_contrat.isoformat()
                    ...
                    "derniere_connexion": (
                        self.derniere_connexion.isoformat()
            ...
            data.update(
                {
                    ...
                    "intervention_en_cours": (
                        self.intervention_en_cours.to_dict()
                    ...
                    "interventions_en_retard": [
                        i.to_dict() for i in self.get_interventions_en_retard()
                    ],
