...
class ReportFilters(BaseModel):
    ...
    period: Optional[ReportPeriod] = Field(
        ReportPeriod.LAST_30_DAYS, description="Période prédéfinie"
    )
...
class ReportRequest(BaseModel):
    ...
    filters: Optional[ReportFilters] = Field(
        default_factory=ReportFilters, description="Filtres appliqués"
    )
    ...
    email_recipients: Optional[List[str]] = Field(
        None, description="Destinataires email"
    )
