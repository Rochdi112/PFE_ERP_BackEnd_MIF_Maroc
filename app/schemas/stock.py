...
class PieceDetacheeBase(BaseModel):
    ...
    reference: str = Field(
        ..., min_length=1, max_length=100, description="Référence unique"
    )
    ...
    prix_unitaire: Optional[Decimal] = Field(
        None, ge=0, max_digits=10, decimal_places=2, description="Prix unitaire"
    )
    cout_achat: Optional[Decimal] = Field(
        None, ge=0, max_digits=10, decimal_places=2, description="Coût d'achat"
    )
    devise: str = Field("EUR", max_length=3, description="Devise")

    # Fournisseur
    fournisseur: Optional[str] = Field(
        None, max_length=255, description="Nom du fournisseur"
    )
    reference_fournisseur: Optional[str] = Field(
        None, max_length=100, description="Référence fournisseur"
    )

    # Localisation
    emplacement: Optional[str] = Field(
        None, max_length=100, description="Emplacement de stockage"
    )
    ...
    def validate_reference(cls, v):
        ...
        if not v.replace("-", "").replace("_", "").isalnum():
            raise ValueError(
                "La référence ne peut contenir que des lettres, chiffres, "
...
class MouvementStockCreate(MouvementStockBase):
    ...
    intervention_id: Optional[int] = Field(
        None, gt=0, description="ID de l'intervention liée (optionnel)"
    )
...
class InterventionPieceBase(BaseModel):
    ...
    commentaire: Optional[str] = Field(
        None, description="Commentaire sur l'utilisation"
    )
...
class StockAlert(BaseModel):
    ...
    message: str
    priorite: int = Field(
        ..., ge=1, le=5, description="Priorité de l'alerte (1=max, 5=min)"
    )
...
class StockSearch(BaseModel):
    ...
    query: Optional[str] = Field(
        None, description="Recherche textuelle (nom, référence)"
    )
    ...
    statut_stock: Optional[StatutStock] = Field(
        None, description="Filtrer par statut de stock"
    )
