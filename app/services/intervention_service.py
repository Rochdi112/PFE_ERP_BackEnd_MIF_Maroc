...
def create_intervention(
    db: Session, data: InterventionCreate, user_id: int
) -> Intervention:
    ...
    if data.technicien_id:
        if not db.query(Technicien).filter(Technicien.id == data.technicien_id).first():
            raise HTTPException(
                status_code=404, detail="Technicien assigné introuvable"
            )
    equipement = (
...
def update_statut_intervention(
    ...
    if (
        new_statut == StatutIntervention.archivee
        and intervention.statut != StatutIntervention.cloturee
    ):
        raise HTTPException(
            status_code=409, detail="Transition vers 'archivee' interdite"
        )
...
def create_intervention_from_planning(db: Session, planning: Planning) -> Intervention:
    ...
    """
    equipement = (
        db.query(Equipement).filter(Equipement.id == planning.equipement_id).first()
    )
    if not equipement:
        raise HTTPException(
            status_code=404, detail="Équipement introuvable pour le planning"
        )

    intervention = Intervention(
        titre=f"Maintenance préventive - {equipement.nom}",
        description=(
            f"Intervention générée automatiquement depuis le planning #{planning.id}"
        ),
