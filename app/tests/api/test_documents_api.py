...
def test_upload_and_delete_document_flow(
    client, db_session, admin_token, tmp_upload_dir
):
    ...
    eq = create_equipement(
        db_session,
        EquipementCreate(
            nom="DOCAPI", type="t", localisation="L", frequence_entretien="7"
        ),
    )
    user = ensure_user_for_email(
        db_session, email="docapi@example.com", role=UserRole.admin
    )
    ...
    r = client.post(
        f"/documents/?intervention_id={interv.id}", files=files, headers=headers
    )
