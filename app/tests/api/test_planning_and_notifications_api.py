...
def test_create_planning_and_generate_intervention(client, responsable_token):
    ...
    r2 = client.post(
        f"/planning/{pid}/create_intervention",
        headers={"Authorization": f"Bearer {responsable_token}"},
    )
...
def test_notification_creation_and_failure_branch(monkeypatch, client, admin_token):
    ...
    fake_notif = SimpleNamespace(
        type_notification=SimpleNamespace(value="unknown_template"), contenu="c"
    )
    with pytest.raises(HTTPException):
        send_email_notification("noone@example.com", fake_notif)

    # ensure API can create a notification (may return 201 or 404/422
