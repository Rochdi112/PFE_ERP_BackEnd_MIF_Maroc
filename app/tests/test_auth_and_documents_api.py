...
def setup_test_user():
    with SessionLocal() as db:
        data = UserCreate(username="apiadmin", full_name="API Admin", email="admin@example.com", role=UserRole.admin, password="adminpass")
        try:
...
def test_token_endpoint_and_docs_list():
    ...
    resp = client.post("/auth/token", data={"email": "admin@example.com", "password": "adminpass"})
    ...
    # Attempt to list documents without auth should be 401 or 403 (depends RBAC). We'll at least hit the route.
