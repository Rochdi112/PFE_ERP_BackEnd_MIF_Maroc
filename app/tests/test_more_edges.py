...
def test_authenticate_user_wrong_password_raises():
    with SessionLocal() as db:
        data = UserCreate(username="pwuser", full_name="PW", email="pw@example.com", role=UserRole.client, password="rightpwd")
        try:
            create_user(db, data)
        except Exception:
            pass
        with pytest.raises(Exception):
            authenticate_user(db, "pw@example.com", "wrongpwd")


def test_auth_me_endpoint_returns_user():
    with SessionLocal() as db:
        data = UserCreate(username="meuser", full_name="Me", email="me@example.com", role=UserRole.responsable, password="mpass")
        try:
            u = create_user(db, data)
        except Exception:
            u = db.query(type(data)).filter_by(email="me@example.com").first()
...
def test_equipement_delete_conflict_when_interventions_exist():
    with SessionLocal() as db:
        eq = create_equipement(db, EquipementCreate(nom="DELTEST", type="t", localisation="L", frequence_entretien="7"))
        user = ensure_user_for_email(db, email="dconf@example.com", role=UserRole.responsable)
...
def test_delete_document_service_function_removes_file_and_db(tmp_path):
    ...
    with SessionLocal() as db:
        eq = create_equipement(db, EquipementCreate(nom="DOC-EQ", type="t", localisation="L", frequence_entretien="7"))
        user = ensure_user_for_email(db, email="docdel@example.com", role=UserRole.admin)
        ic = InterventionCreate(titre="t", description="d", type_intervention="corrective", statut=StatutIntervention.ouverte, priorite="normale", urgence=False, date_limite=None, technicien_id=None, equipement_id=eq.id)
        interv = create_intervention(db, ic, user_id=user.id)
