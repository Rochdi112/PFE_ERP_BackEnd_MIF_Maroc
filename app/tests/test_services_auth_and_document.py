...
def test_authenticate_user_flow():
    with SessionLocal() as db:
        ...
        try:
            ...
            pass
        tok = authenticate_user(db, "auth@example.com", "Pw12345678!")
...
def test_save_uploaded_file_and_create_document(tmp_path):
    ...
    # create_document requires an intervention; create a minimal intervention
    # via services
    ...
    with SessionLocal() as db:
        ...
        eq = create_equipement(
            db,
            EquipementCreate(
                nom="TST-EQ", type="t", localisation="L", frequence_entretien="7"
            ),
        )
        # ensure user
