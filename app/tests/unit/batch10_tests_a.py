...
def test_upload_alias_calls_create(monkeypatch):
    with patch('app.api.v1.documents.create_document', return_value={'id':1}) as cd:
        res = documents_router.upload_document_alias(intervention_id=1, file=MagicMock(), db=None)
        assert res == {'id':1}
...
def test_delete_document_file_removed(tmp_path, monkeypatch):
    ...
    u = User(username='u2', email='u2@example.com', hashed_password='x', role=UserRole.admin)
...
def test_send_email_notification_success(mock_env, mock_smtp):
    ...
    class DummyType: value = 'information'
    dummy = MagicMock()
...
def test_document_service_create(monkeypatch):
    with patch('app.services.document_service.create_document', return_value={'id':2}) as cd:
        res = documents_router.upload_document(intervention_id=1, file=MagicMock(), db=None)
        assert res == {'id':2}
        cd.assert_called_once()

# 9) Simple test for Notification model resume property
def test_notification_resume_property():
    n = Notification(type_notification=Notification.type_notification.type, canal=Notification.canal, contenu='x'*100, user_id=1, intervention_id=1)
