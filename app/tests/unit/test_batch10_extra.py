...
def test_upload_document_calls_service(monkeypatch):
    with patch('app.api.v1.documents.create_document', return_value={'id': 1}) as cd:
        res = documents_router.upload_document(intervention_id=1, file=MagicMock(), db=MagicMock())
        assert res == {'id': 1}
...
def test_upload_document_alias_calls_service(monkeypatch):
    with patch('app.api.v1.documents.create_document', return_value={'id': 2}) as cd:
        res = documents_router.upload_document_alias(intervention_id=1, file=MagicMock(), db=MagicMock())
        assert res == {'id': 2}
        cd.assert_called_once()
...
def test_notification_send_email_success(monkeypatch):
    ...
    with patch('app.services.notification_service.env') as mock_env, patch('smtplib.SMTP') as mock_smtp:
        tmpl = MagicMock()
...
def test_create_planning_success(monkeypatch):
    ...
    data = PlanningCreate(frequence='mensuel', prochaine_date=datetime.utcnow(), derniere_date=None, equipement_id=eq.id)
...
def test_get_user_by_id_fallback_raises(monkeypatch):
    ...
    monkeypatch.setattr(user_service, 'SessionLocal', lambda: (_ for _ in ()).throw(Exception('boom')))
    with pytest.raises(HTTPException):
