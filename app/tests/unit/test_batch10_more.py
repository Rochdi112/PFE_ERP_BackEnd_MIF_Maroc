...
from app.services import technicien_service
...
def test_delete_document_exception_ignored(monkeypatch):
    ...
    monkeypatch.setattr('os.path.isfile', lambda x: (_ for _ in ()).throw(Exception('boom')))
...
def test_send_email_notification_success(monkeypatch):
    ...
    class DT: value = 'information'
    n = MagicMock(); n.type_notification = DT(); n.contenu = 'c'
    class DummyTmpl:
        def render(self, **k): return '<p>x</p>'
    monkeypatch.setattr(notification_service, 'env', MagicMock(get_template=lambda x: DummyTmpl()))
    monkeypatch.setattr('smtplib.SMTP', lambda h,p: MagicMock(__enter__=lambda s: s, starttls=lambda: None, login=lambda u,p: None, sendmail=lambda f,t,m: None))
