def test_create_competence_trims_and_returns(db_session):
    ...
    from app.services.technicien_service import create_competence

    comp_in = CompetenceCreate(nom="  soudure  ")
    comp = create_competence(db_session, comp_in)
    assert getattr(comp, "nom", "").strip() == "soudure"
    # Depending on DB transaction isolation the competence may or may not be visible;
    # assert the created object matches
    assert getattr(comp, "nom", "").strip() == "soudure"
...
def test_technicien_update_invalid_disponibilite(client, responsable_token, db_session):
    ...
    u = ensure_user_for_email(
        db_session, email="tech2@example.com", role=UserRole.technicien
    )
    ...
    # send invalid disponibilite string should not raise and should preserve
    # previous value
    r2 = client.put(
        f"/techniciens/{tid}",
        json={"disponibilite": "not_a_valid_one"},
        headers={"Authorization": f"Bearer {responsable_token}"},
    )
