...
class DummyUser:
    def __init__(self, id=1, email="u@example.com", hashed_password="pwd", is_active=True, role="admin"):
        self.id = id
        self.email = email
...
def test_decode_token_invalid(monkeypatch):
    ...
    monkeypatch.setattr(rbac, 'jwt', type('J', (), {'decode': lambda *a, **k: (_ for _ in ()).throw(JWTError())}))
...
def test_get_current_user_fallback(monkeypatch):
    ...
    monkeypatch.setattr('app.core.rbac.decode_token', lambda token: {'sub': '42', 'role': 'responsable'})
...
def test_auth_service_invalid_password(monkeypatch):
    ...
    monkeypatch.setattr('app.services.auth_service.verify_password', lambda pw, hpw: False)
    with pytest.raises(HTTPException):
...
def test_auth_service_disabled_user(monkeypatch):
    ...
    db = DummyDB(user=dummy)
    monkeypatch.setattr('app.services.auth_service.verify_password', lambda pw, hpw: True)
    with pytest.raises(HTTPException):
