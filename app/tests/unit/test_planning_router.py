...
def test_get_and_update_planning(monkeypatch):
    dummy = {'id': 42}
    with patch('app.api.v1.planning.get_planning_by_id', return_value=dummy) as g:
        res = planning_router.get_planning(42, db=None, user={'id':1})
        assert res == dummy

    with patch('app.api.v1.planning.update_planning_dates', return_value=dummy) as u:
        res2 = planning_router.update_planning_next_date(42, datetime.utcnow(), db=None)
        assert res2 == dummy

    with patch('app.services.planning_service.update_planning_frequence', return_value=dummy) as uf:
        res3 = planning_router.update_planning(42, payload={'frequence':'mensuel'}, db=None)
        assert res3 == dummy
