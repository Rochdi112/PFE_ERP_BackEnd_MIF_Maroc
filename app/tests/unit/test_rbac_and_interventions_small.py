...
# Test create_new_intervention branch where user_id is None -> ensure_user_for_email called
...
def test_create_new_intervention_ensures_user(monkeypatch):
    ...
    class Ensured:
        def __init__(self):
            self.id = 123
    monkeypatch.setattr(interventions_module, 'ensure_user_for_email', lambda db, email, role: Ensured())
    ...
    res = interventions_module.create_new_intervention(Data(), db=DummyDB(), user=user)
