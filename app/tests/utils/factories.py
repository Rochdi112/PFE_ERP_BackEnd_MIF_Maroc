...
def create_admin(db):
    try:
        return create_user(db, UserCreate(username="adminf", full_name="Admin F", email="adminf@example.com", role=UserRole.admin, password="adminpass"))
    except Exception:
...
def create_equipement_helper(db, name="EQ-FACT"):
    return create_equipement(db, EquipementCreate(nom=name, type="machine", localisation="Loc", frequence_entretien="30"))
